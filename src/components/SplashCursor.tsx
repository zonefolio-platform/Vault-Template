...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Read accent color from CSS variable, fall back to prop
    const cssAccent = getComputedStyle(document.documentElement)
      .getPropertyValue('--accent').trim();
    const resolvedColor = cssAccent || COLOR;
    ...
    const onTouchStart = (e: TouchEvent) => {
      ...
      for (let i = 0; i < e.targetTouches.length; i++) {
        updatePointerDownData(pointer, e.targetTouches[i].identifier, scaleByPixelRatio(e.targetTouches[i].clientX), scaleByPixelRatio(e.targetTouches[i].clientY));
      }
    };
    ...
    const onTouchMove = (e: TouchEvent) => {
      const pointer = pointers[0];
      for (let i = 0; i < e.targetTouches.length; i++) {
        updatePointerMoveData(pointer, scaleByPixelRatio(e.targetTouches[i].clientX), scaleByPixelRatio(e.targetTouches[i].clientY), pointer.color);
      }
    };

    const onTouchEnd = (e: TouchEvent) => {
      for (let i = 0; i < e.changedTouches.length; i++) updatePointerUpData(pointers[0]);
    };

    // First-move handlers start the render loop once
    const onFirstMouseMove = (e: MouseEvent) => {
      const pointer = pointers[0];
      updatePointerMoveData(pointer, scaleByPixelRatio(e.clientX), scaleByPixelRatio(e.clientY), generateColor());
      updateFrame();
    ...
    document.body.addEventListener('touchstart', onFirstTouchStart);
